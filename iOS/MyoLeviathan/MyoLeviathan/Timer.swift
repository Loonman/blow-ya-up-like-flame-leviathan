...
public class Timer {
	...
	/// Create a new Timer
	public init(timeInterval: Int64, queue: dispatch_queue_t!, repeats: Bool, closure: (() -> ())?) {
	...
	/// Create a new Timer
	public convenience init(timeInterval: Int64, repeats: Bool, closure: (() -> ())?) {
	...
	public convenience init(seconds: Double, repeats: Bool, closure: (() -> ())?) {
		self.init(timeInterval: Int64(seconds * Double(nanoSecondsPerSecond)), queue: dispatch_get_main_queue(), repeats: repeats, closure: closure)
	}
	...
	public func schedule() {
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, self.timeInterval), self.queue) { [weak self] () -> Void in
